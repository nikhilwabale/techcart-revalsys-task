 ...
 *
 * WHY "use client"?
 * - useCart() hook needs CartContext (client-side state)
 * - Dynamic cart items from localStorage — server has no access to this
 * - onClick handlers for remove, clear, quantity update
 * - framer-motion animations need browser DOM
 *
 * HYDRATION IN ACTION HERE:
 * Server sends CartPage HTML → Browser displays it → React hydrates CartClient
 *
 * IMPORTANT — Hydration mismatch prevention:
 * CartContext uses isLoaded flag. Before localStorage is read:
 *   Server renders: items = []  (empty)
 *   Client has:     items = [product1, product2]  (from localStorage)
 *
 * Without isLoaded guard → React throws hydration mismatch error
 * With isLoaded guard    → Server and client both start with [] → then client loads real data
 *
 * This is why you never see a flash of wrong cart count in this app.
 *
 * PERFORMANCE:
 * - totalItems and totalPrice come from useMemo in CartContext
 *   → only recomputed when cart items change, not on every render
 * - motion.article with layout prop → smooth animation when items are added/removed
 * - lg:sticky on order summary → stays visible while scrolling on desktop
 */
    ...
    <section className="container-page">
      ...
      {items.length === 0 ? (
        <div className="rounded-[2rem] bg-white p-10 text-center shadow-sm dark-surface">
          ...
          <Link
            href="/products"
            className="mt-6 inline-flex rounded-2xl bg-blue-700 px-6 py-3 font-black text-white hover:-translate-y-0.5 hover:bg-blue-800"
          >
            Browse Products
          </Link>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
              ...
              >
                ...
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                  <div>
                    ...
                    <p className="mt-3 text-lg font-black text-slate-900 dark-text">
                      ₹{item.price.toLocaleString("en-IN")}
                    </p>
                    <p className="mt-1 text-sm text-slate-500 dark-muted">
          ...
          {/* Order summary — sticky on desktop (performance: no re-render on scroll) */}
          <aside className="h-fit rounded-[2rem] bg-white p-6 shadow-xl shadow-slate-200/80 dark-surface lg:sticky lg:top-28">
            ...
            <div className="mt-6 space-y-4 text-slate-700 dark-muted">
              <div className="flex justify-between">
                ...
                {/* totalItems from useMemo — not recalculated unless cart changes */}
                <span className="font-black">{totalItems}</span>
              </div>
              <div className="flex justify-between">
                <span>Subtotal</span>
                {/* totalPrice from useMemo — not recalculated unless cart changes */}
            ...
            <button
              onClick={clearCart}
              className="mt-3 w-full rounded-2xl bg-slate-100 px-4 py-4 font-black text-slate-700 dark-soft dark-text dark-muted hover:bg-slate-200"
            >
              Clear Cart
            </button>
          </aside>
